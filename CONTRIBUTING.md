...
- **Coverage:** the pure packages (`pkg/golist/`, `pkg/depgraph/`, `pkg/changeset/`) plus the build-metadata package (`internal/project/`) and CLI wiring (`internal/cli/`) must hit **100%** statement coverage, enforced per-package by `scripts/coverage-check.sh` in CI. The Makefile's `coverage-check` target enforces a softer 90% overall floor as a quick local sanity check. `cmd/cascade/` is a one-line entry point and is not separately coverage-gated — its behavior is verified by `internal/cli`'s tests plus an end-to-end binary smoke test.
- **IO seams (function-variable pattern):** any package with a single io edge (`os/exec`, `os.Getwd`, `signal.NotifyContext`, etc.) follows the function-variable seam convention so the io path is testable in-process without spawning subprocesses or sending real OS signals. Declare `var name = realImpl` at package level (file: `seam.go` in the package directory). Tests in `seam_test.go` (file in the same directory but using `package <name>` rather than `package <name>_test` so the unexported variable is reachable) use a `withNameSeam(t, fn)` helper that swaps the impl and restores via `t.Cleanup`. The pattern is currently in use at five seam sites — `pkg/golist`'s `runGoList`, `pkg/changeset`'s `getCwd`, and `internal/cli`'s `runGitDiff` + `runGoListWrapper` + `signalContext` — and is the established cascade-house convention for any future package with an io edge that benefits from in-process test reachability.
